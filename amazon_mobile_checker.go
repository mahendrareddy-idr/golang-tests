...
func main() {
	...
	launched, err := checkAmazonMobileLaunch(url)
	if err != nil {
...
func checkAmazonMobileLaunch(productURL string) (bool, error) {
	html, err := fetchPage(productURL)
	if err != nil {
		return false, err
	}

	if isAmazonProductPage(html) {
		return true, nil
	}
...
func isAmazonProductPage(html string) bool {
	...
	if strings.Contains(lowerHTML, "amazon") == false {
		return false
	}
